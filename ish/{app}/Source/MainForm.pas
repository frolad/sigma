...
Uses
  ...
  // Для работы необходим предустановленыый MS Excel, подключенные модули
     ...
     with MainForm1 do
      begin
        ...
        //
          ...
          begin
            //Новое меню  //Доступность пуктов меню и кнопок
...
//               s:=SigmaLocation+'Source\';
     ...
     //создание класса для поддержания проекта
...
//  обрботчик нажатия кнопки "Экспорт результата в  Excel"
// Открывает для чтения файл "Test.txt" из директории проекта,
// обрабатывает его и экспртирует данные в файл "C:\FromSigma.xls"
// Для работы необходим предустановленыый MS Excel, подключенные модули
                                            // Excel_TLB, ActiveX, ComObj;

procedure TMainForm1.Excel1Click(Sender: TObject);
var
  txtFile : TextFile; // текстовый файл
  Ex      : OleVariant; // *.xls файл
  ClassID : TCLSID;   // переменная для проверки установки Excel
  Rez     : HRESULT;  //  результат, возвращаемый при вышеуказанной проверке

  text    : string; // строка, прочитанная из текстового файла
  Data    : TData; // одномерный массив, в котором хранятся слова
                                                  // из строки text
  Words   : Array of Array of String; // двумерный массив, в котором хранятся
                              // строки и числа из таблиц текстового файла
                              // для записи в файл Excel
  i,j     : Integer;
  count   : Integer;   // счетчик строк текстового файла
  c       : Char;
  buffer  : Double;   // переменная, с помощью которой определяется тип
                                                  // содержимого в строке
...
begin
  ...
  if Rez <> S_OK then
    begin
      MessageDlg('MS EXCEL не установлен, дальнейшая работа невозможна.',
                                                          mtERROR,[mbok],0);
    end;

  // установка '.' разделителем дробной части числа
  DecimalSeparator := '.';
  ...
  // Подсчет числа строк в файле для инициализации динамического массива
  count := 0;
  ...
  Reset(txtFile);    // сброс курсора чтения файла

  // установка размерности массива
  SetLength(Words, count);

  // обработка текстового файла для записи в массив
  count := 0;
  while not Eof(txtFile) do
  begin

      ReadLn(txtFile, text);  // Считывание по одной строке
      c:=#32;                //  ASCII-код пробела (разделяющего символа)

      // получаем массив из строки слов
      Data:=Split(text, c);

      // по 1-му слову необходимо определить содержимое строки - числа таблицы
      // или текст и выделить необходимое кол-во памяти массива Words
        // если текст - записать в массив Words строку text, иначе
       // разбить на числа и записать в массив Words
...
//                  Words[i,j]);
  ...
  CloseFile(txtFile);    // Закрытие текстового файла



  // ********************
  // Работа с текстовым файлом закончена - массив Words сформирован
  // Работа с Excel: выгрузка массива Words
  ...
  Ex.WorkBooks.Close; //  завершение работы с книгой
