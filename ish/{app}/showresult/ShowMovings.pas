...
{    InfoX.text:=cutStr(MyFloatToStr(st[1]));
    ...
    Form1.InfoMax.text:=MyFloatToStr(st[4]);
    Form1.InfoMin.text:=MyFloatToStr(st[5]);
    Form1.InfoConer.text:=MyFloatToStr(st[7]);
    Form1.InfoEcv.text:=MyFloatToStr(st[6]);

    Form1.InfoMoveX.text:=MyFloatToStr_2(move_st[1]);
    Form1.InfoMoveY.text:=MyFloatToStr_2(move_st[2]);
    // Показываем св-ва КЭ
    OneElement:=Elements_Result.GetElement(Element);
    Form1.KENumber.Text  := IntToStr(Element);
    Form1.MUprug.Text    := MyFloatToStr(Materials[OneElement.Material].E);
    Form1.KoefPuas.Text  := MyFloatToStr(Materials[OneElement.Material].Mu);
    Form1.DopNapr.Text   := MyFloatToStr(Materials[OneElement.Material].Sg);
    Form1.Thickness.Text := MyFloatToStr(Materials[OneElement.Material].Thickness);
...
begin
  Form1.ZoneStress.ColCount := 5;
  // задать заголовок таблицы
  Form1.ZoneStress.Cells[0, 0] := 'Зона';
  Form1.ZoneStress.Cells[1, 0] := '№ КЭ';
  Form1.ZoneStress.Cells[2, 0] := 'Полож. Max';
  Form1.ZoneStress.Cells[3, 0] := '№ КЭ';
  Form1.ZoneStress.Cells[4, 0] := 'Отриц. Max';
  ...
  begin
    ...
    Form1.ZoneStress.Cells[1, curZoneNum] := IntToStr(maxPosStressZNum);
    // значение положит
    Form1.ZoneStress.Cells[2, curZoneNum] := MyFloatToStr(maxPosStressZ);
    // номер кэ с макс отриц напряж
    Form1.ZoneStress.Cells[3, curZoneNum] := IntToStr(maxNegStressZNum);
    // значение отриц
    Form1.ZoneStress.Cells[4, curZoneNum] := MyFloatToStr(maxNegStressZ);
  ...
  Form1.ZoneStress.Cells[0, zoneCount + 1] := 'Пластина';
  // номер кэ с макс положит напряж
  Form1.ZoneStress.Cells[1, zoneCount + 1] := IntToStr(maxPosStressPNum);
  // значение положит
  Form1.ZoneStress.Cells[2, zoneCount + 1] := MyFloatToStr(maxPosStressP);
  // номер кэ с макс отриц напряж
  Form1.ZoneStress.Cells[3, zoneCount + 1] := IntToStr(maxNegStressPNum);
  // значение отриц
  Form1.ZoneStress.Cells[4, zoneCount + 1] := MyFloatToStr(maxNegStressP);
