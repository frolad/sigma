...
C          IPR массив управления печатью  
      LOGICAL*1 IPR(50)
      DIMENSION CORD(1),R(1),FORCE1(100),FORCE2(100)
...
* в нем указываются значения нагрузок по оси Х и Y в узлах);      
      ...
      REAL T1*8,T2*8
      INTEGER I,J,TMPI,I1,I2,FORCE1,FORCE2
...
* Обнуление массива сил
        R(NDF*(I-1)+1)=0
        R(NDF*(I-1)+2)=0
* если точка лежит на прямой X=95, то ее номер записывается в FORCE1
        IF (ABS(XT-95).LT.0.01)  THEN
            I1=I1+1
            FORCE1(I1)=I
C            NR=NR+1
        ENDIF
* если точка лежит на прямой Y=1.142857(x-60), то ее номер записывается в FORCE2
...
C            NR=NR+1
        ENDIF
10    CONTINUE
* сортировка содержимого массива FORCE1 по убыванию кординаты Y
      DO 201 I=I1,2,-1
        DO 201 J=1,I-1
            IF (CORD(NDF*(FORCE1(J+1)-1)+2).GT.
     >CORD(NDF*(FORCE1(J)-1)+2)) THEN
                TMPI=FORCE1(J+1)
                FORCE1(J+1)=FORCE1(J)
                FORCE1(J)=TMPI
            ENDIF
...
* сортировка содержимого массива FORCE2 по убыванию кординаты Y
      DO 202 I=I2,2,-1
        DO 202 J=1,I-1
            IF (CORD(NDF*(FORCE2(J+1)-1)+2).GT.
     >CORD(NDF*(FORCE2(J)-1)+2)) THEN
                TMPI=FORCE2(J+1)
                FORCE2(J+1)=FORCE2(J)
                FORCE2(J)=TMPI
            ENDIF
...
* перпендикулярной прямой Y=1.142857(x-60)
        X1=CORD(2*(FORCE2(1)-1)+1)
        Y1=CORD(2*(FORCE2(1)-1)+2)
        X2=CORD(2*(FORCE2(I2)-1)+1)
        Y2=CORD(2*(FORCE2(I2)-1)+2)
        T2=(Y2-Y1)/(X2-X1)
        T2=-1.0/T2         
...
*        RS=RS+S
        X1=X2
        R(NDF*(FORCE2(I)-1)+1)=S*COS(ATAN(T2))
        RS=RS+R(NDF*(FORCE2(I)-1)+1)
        R(NDF*(FORCE2(I)-1)+2)=S*SIN(ATAN(T2))
        NR=NR+1
      IF(IPR(23)) WRITE(6,21) NR,J,CORD(NDF*(J-1)+1),
     >CORD(NDF*(J-1)+2),R((J-1)*NDF+1),R((J-1)*NDF+2)      
300   CONTINUE
      QPR=0
      X1=CORD(2*(FORCE1(1)-1)+1)
      Y1=CORD(2*(FORCE1(1)-1)+2)
      X2=CORD(2*(FORCE1(I1)-1)+1)
      Y2=CORD(2*(FORCE1(I1)-1)+2)
* Распределенная нагрузка, пиложенная к стороне, являющейся частью прямой x=95,
...
* Задание распределенной силы, пиложенной к стороне, являющейся частью прямой x=95
      DO 400 I=1,I1
        J=FORCE1(I)
* Координата текущей точки
        X2=CORD(2*(FORCE1(I)-1)+1)
        Y2=CORD(2*(FORCE1(I)-1)+2)
* Координата предыдущей точки    
        IF (I.NE.1) THEN 
            X1=CORD(2*(FORCE1(I-1)-1)+1)
            Y1=CORD(2*(FORCE1(I-1)-1)+2)
            ELSE
            X1=X2
            Y1=Y2
        ENDIF
* Координата следующей точки
        IF (I.NE.I1) THEN
            X3=CORD(2*(FORCE1(I+1)-1)+1)
            Y3=CORD(2*(FORCE1(I+1)-1)+2)
            ELSE
            X3=X2
            Y3=Y2
        ENDIF
...
* прямой x=95, с учетом того, что он может принадлежать и прямой y=1.142857(x-60)
        RS=RS+R1
        R(NDF*(FORCE1(I)-1)+1)=
     >R(NDF*(FORCE1(I)-1)+1)+R1
        R(NDF*(FORCE1(I)-1)+2)=
     >R(NDF*(FORCE1(I)-1)+2)+0
        QPR=QSL
        NR=NR+1
      IF(IPR(23)) WRITE(6,21) NR,J,CORD(NDF*(J-1)+1),
     >CORD(NDF*(J-1)+2),R((J-1)*NDF+1),R((J-1)*NDF+2)        
...
* Запись значений точечной нагрузки Р
        R(NDF*(FORCE1(1)-1)+1)=R(NDF*(FORCE1(1)-1)+1)+PRM4
        R(NDF*(FORCE2(I2)-1)+2)=R(NDF*(FORCE2(I2)-1)+2)-PRM4
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  21  FORMAT  (2I6,1X,2F10.3,4X,2F9.2,2F9.2)
C   21    FORMAT  (2I6,1X,F11.3,2X,F11.3,4X,F9.2)
 122  RETURN
      IF (NR.NE.0) GOTO 122
